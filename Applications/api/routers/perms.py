...
from schemas.user import PUserCurrent
...
@router.get("/token", response_model=PUserCurrent)
def get_current_active_user(current_user: User = Security(get_current_user, scopes=["user"])) -> User:
...
async def login_for_access_token(form_data: OAuth2PasswordRequestForm = Depends()) -> dict[str, str]:
    ...
    access_token = create_access_token(
            data={"sub": user.uuid, "scopes": user.scopes.split(',')},
