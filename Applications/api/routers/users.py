...
from schemas.user import PUserChangePass, PUserDB, PUserEdit, PUserOut, PUserPass, PUserStatus
...
@router.get("/", response_model=List[PUserOut])
async def get_users():
...
@router.post("/", response_model=PUserOut, status_code=status.HTTP_201_CREATED)
async def create_user(user: PUserPass) -> User:
    return await users.create_user(user)


@router.put("/{uuid}", response_model=PUserDB)
async def update_user(user: PUserEdit, uuid: str,
...
async def change_user_password(passwords: PUserChangePass,
                               current_user: User = Security(get_current_active_user, scopes=["user"])) -> User:
